...
if (!isConfirmation && !isCancellation) {
  ...
  if (contemConfirmacao && !contemCancelamento) {
    ...
    if (senderPn) {
      ...
      // Para listas, usar remoteJidAlt que contém o telefone real
      phoneWithCountryCode = remoteJidAlt.match(/\d+/)?.[0] || '';
      phoneSource = 'remoteJidAlt';
      console.log('📱 Usando remoteJidAlt como fonte do telefone (lista):', phoneWithCountryCode);
    } else if (remoteJid?.includes('@lid')) {
      // Para listas sem remoteJidAlt, não conseguimos identificar o remetente
      console.log('⚠️ Mensagem de lista sem remoteJidAlt - não é possível identificar o remetente.');
      return new Response(JSON.stringify({
        success: true,
