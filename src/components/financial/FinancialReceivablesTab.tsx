...
interface FinancialReceivablesTabProps {
  ...
  filteredAccounts: AccountsReceivable[];
  searchTerm: string;
...
export function FinancialReceivablesTab({
  ...
  const getAccountStatus = (account: AccountsReceivable): string => {
    if (account.receivedDate) return "recebido";
                ...
                <CardFooter className="p-6 pt-0 flex justify-start">
                  <div className="flex items-center space-x-2">
                    <Button
                      variant="default"
                      size="sm"
                      onClick={() => onMarkAsPaid(account.id, account.method || "cash")}
                    >
